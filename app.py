...
def handle_notification():
    ...
    if action == 'accept':
        ...
        booking_collection.update_one({'_id': notification_id}, {'$set': {'status': 'accepted', 'otp': otp, 'order_id': order_id, "order_datetime" : ordered_cdt}})
...
def agent_login():
    ...
    if request.method == 'POST':
        ...
        password = request.form['password']
                ...
                # Check for existing booking
        existing_user = agent_collection.find_one({
            'username': username,
            'password': password
        })

        if existing_user:
            agent_id = existing_user['agent_id']
            if agent_id:
...
def agent_dashboard():
    ...
    agent_name = agent_collection.find({'agent_id': agent_id})
...
def cb_dashboard():
    ...
    registered = cb_collection.find({'cb_id': cb_id})
    registered = list(registered)
    
    # print("agent id is : ", agent_id)

    labours = cb_collection.find({'cb_id': cb_id})
    labour_list = [labour for labour in labours]

    # print("labour_list is :", labour_list )
    return render_template('cb_dashboard.html', cb_id=cb_id, bookings=registered, labours=labour_list)
